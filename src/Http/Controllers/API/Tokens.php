...
use IronGate\CertChief\Http\Controllers\Controller;
...
class Tokens extends Controller
{
    ...
    {
        $token = $this->user()->createToken($request->input('name'));
    ...
    {
        $this->user()->personalAccessTokens()->findOrFail($token)->forceDelete();
